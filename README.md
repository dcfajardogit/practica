...
- Una vez copiados los archivos .jar en sus correspondientes carpetas, se procede a generar la imagen de docker para cada 
  uno de los microservicios (se debe estar posicionado en la carpeta deployment de cada uno de los servicios ya que allí 
  se encuentra el archivo Dockerfile):

  **practica\ms api\deployment>docker build -t ms_api:1.0 -f Dockerfile . practica\ms practica\deployment>docker build -t 
  ms_practica:1.0 -f Dockerfile .**
...
- Luego se procede a generar una nueva imagen de docker para el ms practica que fue el que sufrió cambios en su archivo 
  application.yaml **practica\ms practica\deployment>docker build -t ms_practica:1.0 -f Dockerfile .**
...
- una vez los pods se encuentren arriba se procede a probar ambos microservicios
  Probando por el puerto del service Microservicio backend
  ![image](https://github.com/user-attachments/assets/7967b6ff-4105-4059-9bbb-5127df840306)

  Microservicio consumidor 
  ![image](https://github.com/user-attachments/assets/27d79282-2ece-4b84-8f44-f5a05633ea02)


# Configuración de EKS
- Como primer paso se debe ingresar a la cuenta de AWS y desplegar la plantilla de cloudformation llamada template-    
  reto.yaml que se encuentra en la raíz de este repositorio.
...
- Nos conectamos al cluster de eks por línea de comandos de la siguiente manera:
  ...
  1. aws configure en este punto debemos ingresas las claves de acceso otorgadas para la cuenta de aws
     ![image](https://github.com/user-attachments/assets/1a7b6735-023f-4857-be64-ce111e502b14)


  2. para verificar que estamos conectados a la cuenta ejecutamos lo siguiente:

     **aws sts get-caller-identity**

  3. luego ejecutar se debe conectar al cluster de la siguiente manera:

     **aws eks update-kubeconfig --name MicroservicesCluster --region us-east-1**

     y verificamos con el comando **kubectl get ns**

     ![image](https://github.com/user-attachments/assets/4e7e57fc-c26c-4236-8212-0513bbac01cd)
...
- verificamos con el comando
  ...
  **kubectl get svc**
  ...
  ![image](https://github.com/user-attachments/assets/690222ab-cd85-4be5-9195-829674cef4dd)


- Luego se toma el valor del EXTERNAL-IP del service ms-api-service para reemplazarlo en el archivo application.yaml del   
  ms practica.
...
- Se deben subir las imagenes al repositorio de ECR desplegado en AWS. Para esto debemos obtener la URI del ECR que creamos a través de la iac. en nuestro caso es: **928975674404.dkr.ecr.us-east-1.amazonaws.com/practice-repo**
  ...
  1. Se debe ejecutar el Comando para autenticarse en ecr
...
- Finalmente procedemos a realizar pruebas tanto en línea de comandos como desde el navegador:
  ...
  ![image](https://github.com/user-attachments/assets/f9fabac4-b536-4951-be30-4374d5d31458)

  Navegador

  ![image](https://github.com/user-attachments/assets/d7f694d6-1533-48ff-be07-e407336f1af3)
